...
%
function [COMTY ending] = cluster_jl_mgb_debug_091013(M,s,self,debug,verbose)
...
if (s == 1)
  ...
  while 1
    ...
    [COMt e] = cluster_jl_mgb_debug_091013(Mnew,0,self,debug,verbose);
    if (e ~= 1)
      COMfull = zeros(1,N);
      COMcur = COMt.COM{1};
      for p=1:Ncom
      ...
      [COMfull1 COMSIZE] = reindex_com(COMfull);
      COMTY.COM{k} = COMfull;
